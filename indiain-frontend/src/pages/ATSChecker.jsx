...
export default function ATSChecker({ isDark }) {
  ...
  return (
    <div className={`w-full flex flex-col gap-6 font-sans ${isDark ? "text-slate-200" : "text-slate-800"}`}>
      
      {/* --- Input Section --- */}
      <div className="relative">
        <label className={`block text-sm font-bold mb-2 ml-1 ${isDark ? "text-slate-300" : "text-slate-700"}`}>
          Target Job Skills
        </label>
        ...
        <div className="relative group">
          <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            <svg className={`h-5 w-5 transition-colors ${isDark ? "text-slate-500 group-focus-within:text-purple-400" : "text-slate-400 group-focus-within:text-purple-500"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
            </svg>
          </div>
          <input
            type="text"
            className={`w-full pl-11 pr-4 py-3.5 rounded-xl border transition-all outline-none shadow-sm ${isDark ? "bg-slate-800 border-slate-700 text-white placeholder-slate-500 focus:border-purple-500 focus:ring-4 focus:ring-purple-500/20" : "bg-slate-50 border-slate-200 text-slate-800 placeholder-slate-400 focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10"}`}
        ...
        <p className={`text-xs mt-2 ml-1 ${isDark ? "text-slate-500" : "text-slate-400"}`}>Separate multiple skills with commas</p>
      </div>

      {/* --- Error Message --- */}
      {error && (
        <div className="text-sm text-rose-500 font-medium animate-fade-in flex items-center gap-1.5 ml-1">
      ...
      {score !== null && !isChecking && (
        <div className={`p-5 rounded-2xl border shadow-sm animate-fade-in-up ${isDark ? "bg-slate-800 border-slate-700" : "bg-white border-slate-100"}`}>
          <div className="flex justify-between items-end mb-2">
            <div>
              <p className={`text-sm font-bold uppercase tracking-wider mb-1 ${isDark ? "text-slate-400" : "text-slate-500"}`}>Match Rate</p>
            ...
            <div className={`text-4xl font-black ${getScoreDetails(score).color} tabular-nums`}>
              {score}<span className={`text-2xl ml-1 ${isDark ? "text-slate-600" : "text-slate-400"}`}>%</span>
            </div>
          </div>
          
          {/* Animated Progress Bar */}
          <div className={`w-full h-3 rounded-full overflow-hidden mt-3 ${isDark ? "bg-slate-700" : "bg-slate-100"}`}>
            <div 
